...
export default function UpdateSlotModal({ occupant, occupants, onClose, onUpdate, academicSessionStartDate, academicSessionEndDate }: UpdateSlotModalProps) {
  const [selectedDate, setSelectedDate] = useState<string>(moment(occupant.scheduledDate).format("YYYY-MM-DD"));
  const [startTime, setStartTime] = useState<string>(typeof occupant.startTime === "string" ? occupant.startTime : moment(occupant.startTime).format("HH:mm"));
  const [endTime, setEndTime] = useState<string>(typeof occupant.endTime === "string" ? occupant.endTime : moment(occupant.endTime).format("HH:mm"));
      ...
      if (newEndTime.isAfter(originalEndTime)) {
        conflictingOccupants = sameDateOccupants.filter((otherOccupant) => {
          const otherStartTime =
            typeof otherOccupant.startTime === "string" ? moment(`${selectedDate} ${otherOccupant.startTime}`) : moment(otherOccupant.startTime);
          const otherEndTime = typeof otherOccupant.endTime === "string" ? moment(`${selectedDate} ${otherOccupant.endTime}`) : moment(otherOccupant.endTime);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const updateData = {
        ...
        subRoomID: occupant.subroomId,
        isAllocationActive: true,
        remarks: remarks,
      };

      const response = await callApi(process.env.NEXT_PUBLIC_UPDATE_SPACE_ALLOCATION_ENTRY || URL_NOT_FOUND, updateData);
      ...
      <div className="relative w-full max-w-lg bg-white rounded-xl shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6">
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-sm space-y-1">
              <p>
                <span className="text-gray-500">Occupant:</span> {occupant.occupantName}
              </p>
              <p>
                <span className="text-gray-500">Type:</span> {occupant.type}
              </p>
              <p>
                <span className="text-gray-500">Date:</span> {moment(occupant.scheduledDate).format("dddd, MMM D, YYYY")}
              </p>
              <p>
                <span className="text-gray-500">Time:</span> {startTime} - {endTime}
              </p>
            </div>
          </div>
          ...
          {isSlotFullyElapsed() && (
            <div className="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
              ...
              <p className="text-sm text-gray-600">This slot has already ended. You can view the details but cannot make changes.</p>
            </div>
          )}
          ...
          {isSlotPartiallyElapsed() && !isSlotFullyElapsed() && !isCurrentOngoingSlot() && (
            <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              ...
              <p className="text-sm text-yellow-600">This slot is currently in progress. You can only edit the remaining time.</p>
            </div>
          )}
          ...
          {isCurrentOngoingSlot() && (
            <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
              ...
              <p className="text-sm text-red-600">This slot is currently ongoing and cannot be updated. You can only unallocate the remaining time.</p>
            </div>
          )}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                {!isSlotFullyElapsed() && !isSlotPartiallyElapsed() && <p className="text-xs text-gray-500 mt-1">Select any time between 09:00 and 18:00</p>}
              </div>
              ...
              <div>
                ...
                {!isSlotFullyElapsed() && !isSlotPartiallyElapsed() && <p className="text-xs text-gray-500 mt-1">Must be after start time</p>}
              </div>
            </div>
            ...
            {!isSlotFullyElapsed() && !isCurrentOngoingSlot() && (
              <div>
                {!showRemarks ? (
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Remarks (Why is this slot being updated?)</label>
                    <textarea
                      value={remarks}
                    ...
                    <div className="flex justify-between items-center mt-2">
                      <p className="text-xs text-gray-500">Remarks are required to update the slot</p>
                      <button
                        onClick={() => {
            ...
            {conflictInfo.hasConflict && (
              <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                ...
                <p className="text-sm text-red-600 mb-2">The selected time slot conflicts with existing allocations:</p>
            ...
            {selectedDate && startTime && !validateTimeNotInPast() && (
              <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                ...
                <p className="text-sm text-red-600">Cannot schedule a time in the past.</p>
              </div>
            )}
            ...
            {!conflictInfo.hasConflict && selectedDate && startTime && endTime && validateTimeRange() && validateTimeNotInPast() && (
              <div className="p-4 bg-orange-50 border border-orange-200 rounded-lg">
                ...
                <p className="text-sm text-orange-600 mt-1">
                  ...
                  {remarks.trim() && <span className="block mt-1 text-green-600">✓ Remarks provided - ready to update</span>}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="flex-shrink-0 px-8 py-6 border-t border-gray-200">
          <div className="flex justify-end gap-3">
            <button onClick={onClose} className="px-4 py-2 bg-gray-200 rounded-lg text-gray-700 hover:bg-gray-300 transition" disabled={isUpdating}>
